...
def acessar_anuncios_inativos(browser):
    ...
    # Tentando uma abordagem alternativa para garantir que a página tenha sido carregada
    try:
        wait = WebDriverWait(browser, 20)  # Aumenta o tempo de espera
        # Mudança de seletor para algo mais genérico ou que já esteja visível na página
        wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, '[data-testid="ad-listing-container"]')))
        wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, '[data-testid="ad-listing-container"]')))
        print('[✅] Página carregada com sucesso!')
        return True  # Se a página foi carregada corretamente, retorna True
    except Exception as e:
        print(f'[❌] Não carregou corretamente. Erro: {e}')
        return False
